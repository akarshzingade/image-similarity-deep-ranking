...
def _list_valid_filenames_in_directory(directory, white_list_formats,
    ...
    """
    def _recursive_list(subpath):
        #print ("started in recursive_list function")
        ...
        #wrong_triplet =0 
        filenames = []
        #print ("len of triplets"+str(len(triplets)))
        #print (triplets[:10])
            #all_files = set(os.listdir(os.path.abspath(subpath)))
        #print (all_files[:10])
        #print ("len of all_files"+str(len(all_files)))
        #wrong_triplet
        #wrong_triplet  = 0
        for triplet in triplets:
            ...
            for files in all_files:
                    if files.split('.')[0]==triplet[0].strip():
                        triplet[0] = triplet[0].strip()+"."+files.split('.')[1]
                        continue
                    if files.split('.')[0]==triplet[1].strip():
                        triplet[1] = triplet[1].strip()+"."+files.split('.')[1]
                        continue
                    if files.split('.')[0]==triplet[2].strip():
                        triplet[2] = triplet[2].strip()+"."+files.split('.')[1]
                        continue
            '''
            filenames +=[triplet[0]]
            filenames +=[triplet[1]]
            filenames +=[triplet[2]]
        #print ("len of filenames after triplets"+str(len(filenames)))
        #print ("Number of wrong triplets: "+str(wrong_triplets))
        to_return_tuple= tuple()
        to_return_tuple = (os.path.abspath(subpath),[],filenames,)
        #print ("done with recursive_list function")
...
class DirectoryIterator(Iterator):
    ...
    def __init__(self, directory, image_data_generator,triplet_path,
        ...
        for res in results:
            ...
            i += len(classes)
        #print ("len of filenames",len(self.filenames))
        pool.close()
        pool.join()
...
def convnet_model_():
    #model = load_model('./cifar10vgg.h5')
    #return model
...
deep_rank_model.load_weights('deepranking_adam_100_epoch.h5')
